...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const ultramanData = {
        'm78_l77': [
            ...
            { name: 'ウルトラマン　スコット', image: 'image/SCOTT.webp', description: 'ウルトラ小隊「Ultra Force」の隊長', detailPage: 'ultraman_detail.html?name=scott' },
            { name: 'ウルトラマン　チャック', image: 'image/CHUCK.webp', description: 'ウルトラ小隊のメンバーの一人', detailPage: 'ultraman_detail.html?name=chuck' },
            { name: 'ウルトラウーマン　ベス', image: 'image/BES.webp', description: 'ウルトラ小隊のメンバーの一人', detailPage: 'ultraman_detail.html?name=beth' },
            { name: 'ウルトラマンキング', image: 'image/KING.webp', description: '伝説のウルトラマン', detailPage: 'ultraman_detail.html?name=king' },
        ],
    ...
    if (ultramanName) {
        fetch(`data/details/${ultramanName}.json`)
            ...
            .then(data => {
                ...
                // 能力
                if (data.abilitiesTitle && data.abilities) {
                    detailHtml += `
                        <h3>${data.abilitiesTitle}</h3>
                        <p>${data.abilities.join('、')}</p>
                    `;
                }

                // 武器
                if (data.weaponsTitle && data.weapons && Array.isArray(data.weapons) && data.weapons.length > 0) {
                    detailHtml += `<h4 class="weapon-title">${data.weaponsTitle}</h4>`;
