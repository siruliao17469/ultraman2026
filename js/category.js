...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (series && ultramanData[series]) {
        // 设置分类标题
        categoryTitleElement.textContent = categoryTitles[series] || 'ウルトラマン';

        const ultramans = ultramanData[series];
        
        if (ultramans.length > 0) {
            ultramans.forEach(ultra => {
        ...
        // 如果没有指定 series 或 series 无效
        categoryTitleElement.textContent = 'カテゴリーが見つかりません';
        ultramanListContainer.innerHTML = '<p>有効なカテゴリーを選択してください。</p>';
    }
});